...
class RunwayDataset3(BasicDataset):
    def __init__(self, images_dir, mask_dir, scale: float = 1.0, mask_suffix: str = '', num_kp = 16):
        self.images_dir = Path(images_dir)
        self.mask_dir = Path(mask_dir)
        assert 0 < scale <= 1, 'Scale must be between 0 and 1'
    ...
    def __getitem__(self, idx):
        ...
        #     processed_mask = self.preprocess(mask, self.scale, is_mask = True)
        #     all_mask[i,:,:] = processed_mask
        mask_file = list(self.mask_dir.glob(f"{name}{self.mask_suffix}.txt"))
        assert len(mask_file) == 1, f'Either no mask or multiple masks found for the ID {name}: {mask_file}'
        mask_fn = mask_file[0]
        data = np.loadtxt(mask_fn, delimiter=',')
        for i in range(data.shape[0]):
            mask = Image.fromarray(self._convertToHM(img.size[1], img.size[0], data[i,:], sigma=2)*255)
            processed_mask = self.preprocess(mask, self.scale, is_mask = True)
            all_mask[i,:,:] = processed_mask
    ...
    def _convertToHM(self, H, W, keypoints, sigma=5):
        ...
        img_hm = np.zeros(shape=(H, W), dtype=np.float32)

        # for i in range(0, nKeypoints // 2):
        x = keypoints[0]
        y = keypoints[1]

        img_hm = self._gaussian(x, y, sigma, H, W)
